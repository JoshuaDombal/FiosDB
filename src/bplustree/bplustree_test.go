...
//		rand.Shuffle(len(tuplesToInsert), func(i, j int) { tuplesToInsert[i], tuplesToInsert[j] = tuplesToInsert[j], tuplesToInsert[i] })
//		rand.Shuffle(len(tuplesToDelete), func(i, j int) { tuplesToDelete[i], tuplesToDelete[j] = tuplesToDelete[j], tuplesToDelete[i] })
//
//		// insert all tuples
//		for idx, pair := range tuplesToInsert {
//			bpt.Set(pair.key, pair.value)
...
//		hitCount := 0
//		for i := 1; i <= 12; i++ {
//			if _, ok := casesHit[i]; !ok {
//				log.Println(fmt.Sprintf("missed: %d", i))
//			} else {
//				hitCount++
//			}
//		}
//
//		log.Println(fmt.Sprintf("hit percent: %v", float32(hitCount) / 12.0))
//
//		if hitCount == 12 {
//			log.Println(fmt.Sprintf("tuplesToInsert: %v", tuplesToInsert))
//			log.Println(fmt.Sprintf("tuplesToDelete: %v", tuplesToDelete))
//			panic(1)
//		}
//	}
//
